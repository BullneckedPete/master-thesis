generateData <- function(n, p, rho, sd, beta_star, type) {
  ...
  genYlinear <- function(X, beta_star, sd, n) {
    e <- rnorm(n, mean = 0, sd = sd);
    return( t(X) %*% beta_star + e );
  }
  
  genYlogistic <- function(X, beta_star) {
  ...
  if (type == "linear") {
    y <- genYlinear(X = X, beta_star = beta_star, sd = sd, n = n);
  } else if (type == "logistic") {
    y <- genYlogistic(X =X, beta_star = beta_star);
  }
